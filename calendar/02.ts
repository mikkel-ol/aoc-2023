const input = `Game 1: 12 red, 2 green, 5 blue; 9 red, 6 green, 4 blue; 10 red, 2 green, 5 blue; 8 blue, 9 red
...
const part1 = input
  ...
  .map<Game>((str) => ({
    ...
    rounds: str
      ...
      .map<Round>((x) => ({
        red: Number(x.find((y) => y.includes("red"))?.split(" ")[0] ?? 0),
        green: Number(x.find((y) => y.includes("green"))?.split(" ")[0] ?? 0),
        blue: Number(x.find((y) => y.includes("blue"))?.split(" ")[0] ?? 0),
      })),
  }))
  .filter(
    (x) =>
...
const part2 = input
  ...
  .map((game) => ({
    red: game.rounds.reduce(
      (acc, next) => (next.red > acc ? next.red : acc),
      0
    ),
    green: game.rounds.reduce(
      (acc, next) => (next.green > acc ? next.green : acc),
      0
    ),
    blue: game.rounds.reduce(
      (acc, next) => (next.blue > acc ? next.blue : acc),
      0
    ),
  }))
  .reduce((acc, next) => acc + next.red * next.green * next.blue, 0);
