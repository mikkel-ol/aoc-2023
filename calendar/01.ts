...
const part1 = input
  ...
  .map((line) => {
    ...
    return Number(`${numbers.first()}${numbers.last()}`);
  })
...
const part2 = input
  ...
  .map((line) => {
    ...
    for (let i = 0; i < line.length; i += 1) {
      const windowThree = `${line[x0]}${line[x1]}${line[x2]}`;
      const windowFour = `${line[x0]}${line[x1]}${line[x2]}${line[x3]}`;
      const windowFive = `${line[x0]}${line[x1]}${line[x2]}${line[x3]}${line[x4]}`;

      if (!first) {
        first = isNaN(Number(line[x0]))
          ? numberMap.get(windowThree) ||
            numberMap.get(windowFour) ||
            numberMap.get(windowFive)
          : Number(line[x0]) ||
            Number(line[x1]) ||
            Number(line[x2]) ||
            Number(line[x3]) ||
      ...
      last =
        isNaN(Number(line[x4])) &&
        isNaN(Number(line[x3])) &&
        isNaN(Number(line[x2])) &&
        isNaN(Number(line[x1])) &&
        isNaN(Number(line[x0]))
          ? numberMap.get(windowFive) ||
            numberMap.get(windowFour) ||
            numberMap.get(windowThree) ||
            last
