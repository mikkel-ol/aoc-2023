...
const hasAdjacentSymbols = (
  start: [number, number], // [row, column]
  length: number
) => {
  ...
  for (let i = 0; i < length; i++) {
    for (let rowI = -1; rowI <= 1; rowI++) {
      for (let columnI = -1; columnI <= 1; columnI++) {
        const symbol = matrix[row + rowI]?.[column + columnI + i];

        if (isSymbol(symbol)) return true;
      }
    }
  }
...
for (let i = 0; i < matrix.length; i++) {
  for (let j = 0; j < matrix[i].length; j++) {
    ...
    if (Number(cell)) {
      ...
      while (!isNaN(Number(row[j + 1]))) {
        num = `${num}${row[j + 1]}`;
        j += 1;
      }

      // we have our number
      if (hasAdjacentSymbols([i, j - num.length + 1], num.length)) {
...
const part1 = numbers.reduce((acc, next) => acc + next);
...
const isGear = (
  ...
  for (let rowI = -1; rowI <= 1; rowI++) {
    for (let columnI = -1; columnI <= 1; columnI++) {
      const num = matrix[row + rowI]?.[column + columnI];
